...
package it applies to a reviewed agent that orchestrates such a workflow.

Artifact scope for this package is set by
[Agent roast contract](./10-agent-roast-contract.md), which supersedes this
...
Artifact scope for this package is set by
[Agent roast contract](./10-agent-roast-contract.md), which supersedes this
lens's own artifact-scope and reroute statements. Applying the lens to an agent
definition is correct here, and the lens's own advice to hand agent workflows
to Skill Coach is never reported as a routing defect. Every safety boundary
below still applies.
