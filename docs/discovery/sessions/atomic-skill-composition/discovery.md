...
- atoms live at `skills/_base/_atoms/<name>/<name>.md` and molecules at
  `skills/_base/_molecules/<name>/<name>.md`, so the level is derivable from the
  path and every unit's support files are isolated in its same-named root.
  Non-routability is inherited from the existing `_base` exclusion. Since
...
- Maturity: promotion-ready
- Priority: P0
- Outcome: Atoms live at `skills/_base/_atoms/<name>/<name>.md` and molecules at `skills/_base/_molecules/<name>/<name>.md`. Both are single Markdown units isolated in same-named roots with their support files. Address is the **sole** authority for level. An atom declares `includes: []`; a molecule declares two or more downward composition references. Scripts and tests are co-located by basename inside the unit root. Unit names are `<domain>-<verb>[-<object>].md`, kebab-case, globally unique, no numeric prefix and no level suffix; and every unit declares `## Inputs`, `## Output`, `## Guarantees`, and `## Boundaries`. The post-c-0006 user directive superseded the flat-namespace decision without renaming units.
- Open questions: none.
