...
| --- | --- |
| `Complete` | Every mandatory lens reviewed the staged evidence and the report is final. |
...
| `Stale evidence` | The package changed between staging and synthesis, so the findings no longer describe the live files. |
| `Awaiting artifact` | No readable skill package or no valid input was supplied. |
| `Unsupported artifact type` | The target is not a skill package. |
...
- **`Stale evidence`** — name the changed entries. Recovery: stop editing the
  package and rerun this skill against the settled files.
...
- **`Unsupported artifact type`** — name the supplied target and route it:
  a single agent file to `roast-this-agent`, a single prompt to
...
checklist in [Skill roast contract](./10-skill-roast-contract.md).
